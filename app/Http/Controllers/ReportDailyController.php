...
class ReportDailyController extends Controller
{
    ...
    private function getReportDailyData($today)
    {
            return DB::table('users')
                ...
                ->leftJoin('customer_types', 'customer_types.id', '=', 'leads.customer_type_id')
                ->leftJoin('customer_statuses', 'customer_statuses.id', '=', 'leads.current_customer_status_id')
                ->select(
                    'users.id',
                    'users.name as sale_name',
                    ...
                    DB::raw('SUM(
                        CASE
                            WHEN customer_types.name = "Khách Hàng Mới"
                            THEN 1 ELSE 0
                        END
                    ...
                    DB::raw('SUM(
                        CASE
                            WHEN customer_types.name = "Khách Hàng Cũ"
                            THEN 1 ELSE 0
                        END
                    ) as total_old_customers'),
                    ...
                    DB::raw('SUM(
                        CASE
                            ...
                            THEN 1 ELSE 0
                        END
                    ) as total_old_potential'),

                    DB::raw('SUM(
                        CASE
                            WHEN customer_statuses.name = "Tiềm Năng"
                            THEN 1 ELSE 0
                        END
                    ) as total_potential'),
                    ...
                    DB::raw('SUM(
                        CASE
                            WHEN customer_statuses.name = "Quan Tâm"
                            THEN 1 ELSE 0
                        END
                    ) as total_care'),
                    ...
                    DB::raw('COALESCE(SUM(leads.order_value),0) as total_value')
                )
                ->groupBy('users.id', 'users.name')
                ->orderBy('users.name')
                ->get();
    ...
    private function getReportMonthData($from, $to)
    {
        return DB::table('users')
            ->leftJoin('leads', function ($join) use ($from, $to) {
                $join->on('users.id', '=', 'leads.sale_information_id')
    ...
    public function exportCurrentMonth(Request $request)
    {
        ...
        // Khoảng 1
        $fromDate1 = $request->filled('from_date_1')
            ? Carbon::parse($request->from_date_1)->startOfDay()
            : Carbon::now()->startOfMonth();

        $toDate1 = $request->filled('to_date_1')
            ? Carbon::parse($request->to_date_1)->endOfDay()
            : Carbon::now()->endOfMonth();

        // Khoảng 2
        $fromDate2 = $request->filled('from_date_2')
            ? Carbon::parse($request->from_date_2)->startOfDay()
            : Carbon::now()->subMonth()->startOfMonth();

        $toDate2 = $request->filled('to_date_2')
            ? Carbon::parse($request->to_date_2)->endOfDay()
            : Carbon::now()->subMonth()->endOfMonth();

        $data = $this->getReportMonthData($fromDate1, $toDate1);
        $data_prev = $this->getReportMonthData($fromDate2, $toDate2);
    ...
    public function exportCurrentMonthExcel(Request $request)
    {
        abort_unless(auth()->user()->hasPermission('report_month.view'), 403);
        ...
        return Excel::download(
            new ReportMonthExport($data, $data_prev),
            'BaoCaoThang_'.$fromDate1->format('Ymd').'_'.$toDate1->format('Ymd').'.xlsx'
        );
    ...
    private function getReportShowroomData($fromDate1, $toDate1, $fromDate2, $toDate2)
{
    ...
    */
    ...
    // ============================================================
    // THÁNG HIỆN TẠI - KHÁCH ONLINE
    // GIỮ NGUYÊN LOGIC ONLINE
    // ============================================================
    ...
    // ============================================================
    // CỘNG DỮ LIỆU SR THÁNG HIỆN TẠI
    // ============================================================

    foreach ($data_offline as $item) {

        $sr_total_customers += (int) $item->total_customers;
        $sr_new_customers += (int) $item->new_customers;
        $sr_old_customers += (int) $item->old_customers;
        $sr_new_potential += (int) $item->new_potential;
        $sr_old_potential += (int) $item->old_potential;
        $sr_total_care += (int) $item->total_care;
        $sr_total_reference += (int) $item->total_reference;
        $sr_total_no_need += (int) $item->total_no_need;
        $sr_new_closed += (int) $item->new_closed;
        $sr_old_closed += (int) $item->old_closed;
        $sr_total_new_closed += (float) $item->total_new_closed;
        $sr_total_old_closed += (float) $item->total_old_closed;

        $sr_total_sale_closed +=
            (float) $item->total_new_closed +
            (float) $item->total_old_closed;

        $sr_total += (float) $item->total;
    ...
    foreach ($showrooms as $showroom) {
        ...
        foreach ($sources as $source_id => $source_name) {
            ...
            if (
                $showroomNorm &&
                strpos($sourceNorm, $showroomNorm) !== false
            ) {
                $sourceIdToShowroomId[$source_id] = $showroom->id;
            }
        }
    ...
    $totals_sum = [
        ...
        'sr_new_potential' => $sr_new_potential,
        'sr_old_potential' => $sr_old_potential,
        'sr_total_potential' => $sr_new_potential + $sr_old_potential,
        'sr_total_care' => $sr_total_care,
        'sr_total_reference' => $sr_total_reference,
        'sr_total_no_need' => $sr_total_no_need,
        'sr_new_closed' => $sr_new_closed,
    ...
    // ============================================================
    // KHỞI TẠO THÁNG TRƯỚC THEO SHOWROOM
    // ============================================================
    ...
    public function reportShowroom(Request $request)
    {
        ...
        $result = $this->getReportShowroomData(
            $fromDate1,
            $toDate1,
            $fromDate2,
            $toDate2
        ...
        return view('report_showroom.index', $result);
    }
    public function exportShowroomExcel(Request $request)
    {
        abort_unless(auth()->user()->hasPermission('report_showroom.view'), 403);

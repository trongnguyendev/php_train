...
<div class="container-fluid">
    <div class="d-flex justify-content-between mb-3">
        ...
        <a href="{{ route('customer_status.create') }}" class="btn btn-primary">
    ...
    <div class="card">
        ...
        <div class="card-body">
            <table class="table table-hover">
                ...
                <tbody>
                    @forelse($customerStatus as $item)
                        <tr>
                            <td>{{ $item->name }}</td>
                            <td class="text-end">
                                <a href="{{ route('customer_status.show', $item) }}" class="btn btn-sm btn-info">
                                    <i class="bi bi-eye"></i>
                                </a>
                                <a href="{{ route('customer_status.edit', $item) }}" class="btn btn-sm btn-warning">
                                    <i class="bi bi-pencil"></i>
                                </a>
                                <form action="{{ route('customer_status.destroy', $item) }}" method="POST" class="d-inline">
                                    @csrf
                                    @method('DELETE')

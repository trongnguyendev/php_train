<div class="container">
  <form action="/product-group/create" method="POST" id="orderForm" class="form-group">
    <div style="display: flex; gap: 20px;">
      ...
      <div style="flex: 1;">
        ...
        <label for="name">Chọn sản phẩm Full Bộ:</label>
          <input type="text" name="name" id="name" value="<?= htmlspecialchars($oldInput['name'] ?? '') ?>">
          <?php if (isset($errors['name'])): ?>
              <p class='error'><?= implode(', ', $errors['name']) ?></p>
          <?php endif; ?>

          <label for="price">Giá:</label>
          <input type="number" name="price" id="price" value="<?= htmlspecialchars($oldInput['price'] ?? '') ?>">

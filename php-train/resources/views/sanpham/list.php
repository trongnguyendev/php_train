...
<div class="search-container">
  ...
  <button class="btn-create">
    <a href="/sanpham/create">+ Tạo mới</a>
  </button>
</div>
  ...
  <?php else: ?>
    <div class="table-responsive">
      <table class="modern-table">
        ...
        <tbody>
          <?php foreach ($sanphams as $index => $emp): ?>
            <tr>
              <td><?= htmlspecialchars($emp[1]) ?></td>
              <td><?= htmlspecialchars($emp[2]) ?></td>
              <td><?= htmlspecialchars($emp[3]) ?></td>
              <td>
                <a href="/sanpham/edit/<?= $emp['id'] ?>" class="icon-btn edit" title="Cập nhật"><i class="fa fa-pen"></i></a>
                <a href="/sanpham/delete/<?= $emp['id'] ?>" class="icon-btn delete" title="Xoá"><i class="fa fa-trash"></i></a>
              </td>
            </tr>
          <?php endforeach; ?>
        </tbody>

<div class="update">
    <form action="/user/edit/<?= $indexData ?>" method="POST">
        ...
        <input type="number" name="phone" id="phone" value="<?php echo htmlspecialchars($userData[2] ?? $oldInput['phone'] ?? ''); ?>">
        <?php if (isset($errors['phone'])): ?>
            <p class='error'><?= implode(', ', $errors['phone']) ?></p>
        <?php endif;  ?>

         <label for="province">Tỉnh:</label>
        <input type="text" name="province" id="province" value="<?php echo htmlspecialchars($userData[3] ?? $oldInput['province'] ?? ''); ?>">
    

        <label for="password">Password:</label>
        <input type="text" name="password" id="password" value="<?php echo intval($userData[4] ?? $oldInput['password'] ?? 0); ?>">
